...
land_files() { # repo
  local repo="$1" stub cfg
  stub="$(_render "$_root/stub/agent-loop.yml")"
  cfg="$(_render "$_root/stub/.smallhours.yml")"
  if [ "$DRY_RUN" -eq 1 ]; then
    if default_branch_protected "$repo"; then
      sh_log "DRY-RUN: $DEFAULT_BRANCH requires PRs — would land stub + config via PR ($ONBOARD_BRANCH)"
    else
      sh_log "DRY-RUN: would push stub + config directly to $DEFAULT_BRANCH"
    fi
    return
  fi
  ...
  if ! default_branch_protected "$repo"; then
    ...
    sh_log "✓ pushed stub + config to $DEFAULT_BRANCH"
    return
  fi
